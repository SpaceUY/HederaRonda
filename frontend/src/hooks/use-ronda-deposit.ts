...
export function useRondaDeposit({ 
  ...
  const isLoading = isApprovalPending || isApprovalConfirming || isDepositPending || isDepositConfirming || isEstimatingGas || isCheckingMembership || membershipLoading;
  ...
  const decodeContractError = useCallback((err: any): string => {
    ...
    try {
      ...
      if (errorName.includes('NotParticipant') || errorMessage.includes('not a participant') || errorData.includes('not a participant')) {
        return 'You are not a participant in this RONDA';
      }
      
      if (errorName.includes('AlreadyDeposited') || errorMessage.includes('already deposited') || errorData.includes('already deposited')) {
        return 'You have already made your deposit for this milestone';
      }
      
      if (errorName.includes('MilestoneNotActive') || errorMessage.includes('milestone not active') || errorData.includes('milestone not active')) {
        return 'This milestone is not currently active for deposits';
      }
      
      if (errorName.includes('InvalidMilestone') || errorMessage.includes('invalid milestone') || errorData.includes('invalid milestone')) {
        return 'Invalid milestone index';
      }
      
      if (errorName.includes('RondaNotRunning') || errorMessage.includes('not running') || errorData.includes('not running')) {
  ...
  const estimateGas = useCallback(async (milestoneIndex: number) => {
    ...
    try {
      ...
      const shouldUseFallback = !decodedError.includes('not a participant') && 
  ...
  }, [address, publicClient, roscaContractAddress, monthlyDepositAmount, isETH, paymentToken, monthlyDepositFormatted, decodeContractError]);
  ...
  const executeDepositFlow = useCallback(async (milestoneIndex: number) => {
    if (!address) {
    ...
    if (!isRondaRunning) {
      ...
      setError(`RONDA is not accepting deposits. Current state: ${currentStateName}. Deposits are only allowed when RONDA is Running.`);
      setStep('error');
      return;
    }
    ...
    if (!hasEnoughBalance) {
      ...
      setError(`Insufficient ${tokenSymbol} balance. Need ${monthlyDepositFormatted} ${tokenSymbol} for deposit.`);
      setStep('error');
      return;
    }
    ...
    try {
      ...
      setStep('checking');
      console.log('🔍 Performing final membership and deposit verification...');
  ...
  useEffect(() => {
    if (isApprovalConfirmed && step === 'approving' && currentMilestone !== null) {
  ...
  }, [isApprovalConfirmed, step, currentMilestone, estimatedGas, roscaContractAddress, monthlyDepositAmount, isETH, writeDeposit]);
  ...
  useEffect(() => {
    const currentError = approvalError || depositError || approvalReceiptError || depositReceiptError;
  ...
  }, [approvalError, depositError, approvalReceiptError, depositReceiptError, step, decodeContractError]);
